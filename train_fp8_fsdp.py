...
from gpt import Fp8GPT, Fp8GPTBlock
from llama import Fp8LLaMA, Fp8LLaMABlock
...
def train(
    ...
    dprint = partial(distrib_print, rank)

    # Configure model
    with open(cfg_path) as f:
        cfg_json = json.load(f)
    if cfg_json['arch_name'] == 'gpt':
        cfg_cls, model_cls, blk_cls = GPTConfig, Fp8GPT, Fp8GPTBlock
    elif cfg_json['arch_name'] == 'llama':
        cfg_cls, model_cls, blk_cls = LLaMAConfig, Fp8LLaMA, Fp8LLaMABlock
    else:
    ...
    if pt_compile:
        dprint(f'Compiling in {compile_mode} mode.')
...
def distrib_print(rank, *args, **kwargs):
    if rank == 0:
        print(*args, **kwargs)
